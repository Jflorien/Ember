...
route shells, schema + RLS, design system applied, the `GameEvent` zod schema
(`src/lib/events/`).

**Not built:** realtime wiring, campaign/character CRUD, the rules engine (validation of a
proposed event against game state — the schema only validates *shape*, not legality), the event
bus, the three surfaces themselves, anything AI.

### Next, in order

1. **Schema → live.** Run the migration against a real Supabase project — note it now requires
   `id text` (ULID, generated by `src/lib/events/id.ts`, not by Postgres) and
   `proposed_by text check (in ('human','model'))` on `events`, both fixed to match this file's
   `GameEvent` type and `src/lib/events/schema.ts`; the two had drifted apart. Verify the RLS leak
...
2. **One event end-to-end.** Two browsers, a button, a Realtime subscription, an event landing
   on both under 150ms, validated through `proposedGameEventSchema` before it commits. Everything
   in P1 is a variation on this.
